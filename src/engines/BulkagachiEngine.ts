...
export class BulkagachiEngine {
  ...
  private weatherChangeInterval = 30000 // 30 seconds initial
  ...
  private weatherLocations = ['camp', 'city', 'mountain', 'club']
  ...
  private getGrowthStage(): string {
    ...
    if (ageInHours < 96) return 'ADULT'  // 22-96 hours (4 days)
    return 'ELDER'  // 96+ hours (lives forever)
  }
  ...
  private canEvolve(): boolean {
    ...
    if (ageInHours >= 8 && ageInHours < 22 && !this.teenType) return true
    if (ageInHours >= 22 && ageInHours < 96 && this.teenType) return true
    if (ageInHours >= 96) return true
    return false
  }
  ...
  tryEvolve(): boolean {
    ...
    if (ageInHours >= 22 && ageInHours < 96 && !this.evolutionPending && this.teenType) {
    ...
    if (ageInHours >= 96 && !this.evolutionPending) {
    ...
    } else if (this.teenType && ageInHours < 96) {
      const hoursLeft = Math.max(0, 22 - ageInHours).toFixed(1)
  ...
  private gameLoop(): void {
    ...
    // Every 2-3 seconds, pause or change direction
    if (this.paceTimer > 2000 + Math.random() * 1000) {
      if (this.isPacing) {
    ...
    if (this.isPacing && !this.isSleeping) {
      ...
      let moveSpeed = this.isGhost ? 0.15 : 0.3
      let maxRange = 15
    ...
    if (!this.isSleeping && !this.isAnimatingBounce) {
      if (this.isGhost) {
        ...
        // Egg tilt - slow side to side rock, intensity increases as egg approaches hatching
        const elapsed = nowMs - this.eggStartTime
        const totalHatchTime = 60 * 60 * 1000  // 60 minutes max
        const progress = Math.min(elapsed / totalHatchTime, 1)
        // Slow tilt: 0.001 rad/sec, amplitude increases with progress
        const tiltAmp = 0.02 + progress * 0.03  // 0.02 to 0.05 radians (subtle pendulum)
        this.eggTilt = Math.sin(nowMs * 0.001) * tiltAmp
        
        // Roll animation from pet (big hop + full 360 rotation)
        if (this.eggRollAngle > 0 || this.bobOffset < 0) {
          // Continue rolling until full 360 (2π ≈ 6.28)
          if (this.eggRollAngle < 6.28) {
            this.eggRollAngle += 0.4  // Fast spin
          }
          this.bobOffset += 0.8  // Hop arc
          if (this.bobOffset > 0) this.bobOffset = 0
        }
        
        // Reset roll when complete
        if (this.eggRollAngle >= 6.28) {
          this.eggRollAngle = 0
          this.bobOffset = 0
        }
  ...
  private updateWeather(): void {
    ...
    if (this.weatherTimer > this.weatherChangeInterval) {
      this.weatherTimer = 0
      this.weatherChangeInterval = 30000 + Math.random() * 30000
